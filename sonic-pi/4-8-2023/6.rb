...
def w_play(n_ote, r_elease) # n as in note
  with_fx :lpf do
    with_synth :prophet do
      play n_ote-24, release: r_elease*2, amp: [1.0, 1.0/r_elease].min, pan: rrand(-1.0, 1.0)
    end
  end
end
...
def w_play_chord(c_hord, r_elease)
  with_synth :dark_ambience do
    play_chord c_hord, release: r_elease, sustain: r_elease*8, amp: 2
  end
end
...
live_loop :a do
  n_beats = 8
  a = generate_events(n_beats, [3, 4])
  b = generate_events(n_beats, [5, 2])
  ##| stop
  with_fx :reverb do
    2.times do
      2.times do
